/**
 * POST /api/cron/ai-compliance
 *
 * 外部 Cron 触发端点 — AI 合规审查批量扫描。
 * 由 cron-job.org 或 GitHub Actions 每 30 分钟调用一次（§7.1）。
 *
 * 执行流程：
 ...
 *   5. 查询最多 2 首 PENDING 歌曲（适配 Vercel 10s 超时，§7.3）
...
import {
  runComplianceCheck,
  checkTokenBudget,
} from '~~/server/services/aiComplianceService'
...
export default defineEventHandler(async (event) => {
  ...
  if (!settings.enableAiCompliance) {
    return { skipped: true, reason: 'AI compliance is disabled in settings' }
  }
  ...
  // ===== 5. 查询待扫描歌曲（PENDING，按提交时间升序，优先处理最早的）=====
  ...
  if (pendingSongs.length === 0) {
    return {
      processed: 0,
      message: 'No pending songs to scan',
      warmupMs,
  ...
  for (const song of pendingSongs) {
    ...
    if (elapsed > 9000) {
      console.warn(`[ai-compliance cron] Time budget exhausted after ${elapsed}ms, stopping early`)
      break
    }
    ...
    try {
      ...
      console.error(`[ai-compliance cron] Song ${song.id} failed:`, e.message)
      results.push({
        songId: song.id,
        status: 'ERROR',
        committed: false,
...
async function warmupNeon(): Promise<number> {
  ...
  if (latency > 1000) {
    console.log(`[ai-compliance cron] Neon cold start detected: ${latency}ms`)
  }
  return latency
}
